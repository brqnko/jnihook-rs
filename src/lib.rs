...
static CLASS_FILE_CACHE: LazyLock<Mutex<HashMap<String, jvmti::bytecode::Classfile>>> =
    LazyLock::new(|| Mutex::new(HashMap::new()));
static HOOKS: LazyLock<Mutex<HashMap<String, Vec<HookInfo>>>> =
    LazyLock::new(|| Mutex::new(HashMap::new()));
static ORIGINAL_CLASSES: LazyLock<Mutex<HashMap<String, jni::objects::JClass<'static>>>> =
    LazyLock::new(|| Mutex::new(HashMap::new()));
static ORIGINAL_CLASS_FILE_CACHE: LazyLock<Mutex<HashMap<String, Vec<u8>>>> =
    LazyLock::new(|| Mutex::new(HashMap::new()));
...
pub unsafe fn jnihook_init(jvm: *mut jni::sys::JavaVM) -> anyhow::Result<()> {
    ...
    if (**jvm).GetEnv.context("Failed to get GetEnv")?(
        jvm,
        jvmti_raw,
...
pub unsafe fn jnihook_attach(
    ...
    if !CLASS_FILE_CACHE.lock().unwrap().contains_key(&class_name) {
        ...
        HOOKS
            ...
            .or_default()
            .push(hook_info.clone());

        (**jni_hook.jvmti_raw)
            .RetransformClasses
            .context("Failed to get RetransformClasses")?(
            jni_hook.jvmti_raw, 1, &class.native_id
        );
    ...
    if !ORIGINAL_CLASSES.lock().unwrap().contains_key(&class_name) {
        ...
        ORIGINAL_CLASS_FILE_CACHE
            .lock()
            .unwrap()
            .insert(class_name.clone(), class_data.clone());
        ...
        for replace in replaces {
            let attr_name = jvmti::bytecode::printer::ClassfilePrinter::resolve_utf8(
                &replace,
                &class_file_cache.get(&class_name).unwrap().constant_pool,
            );
            ...
            replaces2.push((
                replace.idx,
                jvmti::bytecode::classfile::Constant::Utf8(class_copy_source_name.clone().into()),
            ));
        }
        ...
        replaces.iter().for_each(|idx| {
            if class_name
                == jvmti::bytecode::printer::ClassfilePrinter::resolve_utf8(
                    idx,
                ...
                )
            {
                replaces2.push((
                    idx.idx,
                    jvmti::bytecode::classfile::Constant::Utf8(class_copy_name.clone().into()),
                ));
            }
        });
        ...
        for cfi in class_file_cache
            ...
            .iter()
        {
            if let jvmti::bytecode::classfile::Constant::NameAndType {
                name_index,
                descriptor_index,
            } = cfi
            {
                replaces.push((
                    jvmti::bytecode::classfile::ConstantPoolIndex::new(name_index.idx),
                    jvmti::bytecode::classfile::ConstantPoolIndex::new(descriptor_index.idx),
                ));
            }
        }
        ...
        for replace in replaces.iter() {
            let desc = jvmti::bytecode::printer::ClassfilePrinter::resolve_utf8(
                &replace.1,
                &class_file_cache.get(&class_name).unwrap().constant_pool,
            );
            ...
            if desc.contains(&class_desc) {
                ...
                replaces2.push((
                    jvmti::bytecode::classfile::ConstantPoolIndex::new(replace.1.idx),
                    jvmti::bytecode::classfile::Constant::Utf8(new_desc.into()),
                ));
            }
        }
        ...
        for replace in replaces2 {
            class_file_cache
                ...
                .unwrap()
                .constant_pool
                .constants[replace.0.idx] = replace.1;
        }
        ...
        for idx in replaces.iter() {
            let descriptor = jvmti::bytecode::printer::ClassfilePrinter::resolve_utf8(
                &idx.1,
                &class_file_cache.get(&class_name).unwrap().constant_pool,
            );
            ...
            if descriptor.contains(&class_desc) {
                ...
                replaces2.push((
                    jvmti::bytecode::classfile::ConstantPoolIndex::new(idx.1.idx),
                    jvmti::bytecode::classfile::Constant::Utf8(new_desc.into()),
                ));
            }
        }
        ...
        (**jni_hook.jvmti_raw)
            .GetClassLoader
            .context("Failed to get GetClassLoader")?(
            jni_hook.jvmti_raw,
            class.native_id,
            &mut class_loader,
        );

        let class_copy = (**jni_hook.jvm.get_env()?.get_raw())
            .DefineClass
            .context("Failed to get DefineClass")?(
            jni_hook.jvm.get_env()?.get_raw(),
            null(),
        ...
        ORIGINAL_CLASSES.lock().unwrap().insert(
            class_name.clone(),
    ...
    if (method_info.access_flags & jvmti::bytecode::classfile::MethodAccessFlags::Static as i32)
        == jvmti::bytecode::classfile::MethodAccessFlags::Static as i32
    {
        let method_id = env.get_static_method_id(
            original_classes.get(&class_name).unwrap(),
            method_info.name,
...
unsafe fn reapply_class(
    ...
    for (i, idx) in replaces.iter().enumerate() {
        let name = jvmti::bytecode::printer::ClassfilePrinter::resolve_utf8(
            &idx.0,
            &class_file.constant_pool,
        );
        let desc = jvmti::bytecode::printer::ClassfilePrinter::resolve_utf8(
        ...
        if !HOOKS
            ...
            .unwrap()
            .iter()
            .any(|hook| hook.method_info.name == name && hook.method_info.signature == desc)
        {
            continue;
        }

        class_file.methods.get_mut(i).unwrap().access_flags.flags |=
            jvmti::bytecode::classfile::MethodAccessFlags::Native as u16;

        let mut should_remove = Vec::new();
        for (j, attr) in class_file
            .methods
            .get_mut(i)
            .unwrap()
            .attributes
            ...
            .enumerate()
        {
            if matches!(
                attr,
                jvmti::bytecode::classfile::Attribute::Code {
                    max_stack: _,
                    max_locals: _,
                    code: _,
                    exception_table: _,
    ...
    (**jni_hook.jvmti_raw)
        .RedefineClasses
        .context("Failed to get RedefineClasses")?(jni_hook.jvmti_raw, 1, &class_definition);
...
pub unsafe fn jnihook_shutdown() -> anyhow::Result<()> {
    ...
    class_file_cache.iter_mut().for_each(|(k, _)| {
        ...
        (**jni_hook.jvmti_raw)
            .RedefineClasses
            .context("Failed to get RedefineClasses")
            .unwrap()(jni_hook.jvmti_raw, 1, &class_definition);
    });

    jni_hook
        .jvmti
        .set_event_callbacks(jvmti::event::EventCallbacks::default());
